...
- 全选复选框：居中对齐
- 供应商名称：左对齐
- 来源、资料管理、绩效评价、状态、操作：居中对齐

### 7. 状态联动逻辑缺失
**问题：** 状态启用/停用时，资料管理和绩效评价的勾选状态没有联动
**解决：** 实现状态联动逻辑，启用时自动勾选，停用时自动取消勾选
**技术实现：**
- 修改 `updateVendorStatus` 方法，同时更新状态和功能开关
- 更新本地数据和DOM元素
- 通知资料管理模块刷新

### 8. 编辑功能冗余
**问题：** 编辑操作与直接勾选复选框功能重复，增加操作成本
**解决：** 彻底删除编辑按钮和编辑模态框
**技术实现：**
- 删除 `showEditVendorModal` 方法
- 删除 `updateVendorConfig` 方法
- 删除编辑按钮样式和事件绑定
- 操作列只保留删除按钮
