...
class LocalFileSyncService {
  ...
  generateBackupPath(fileInfo) {
    ...
    const backupDir = path.join('资料档案', '_backup', today);
...
### 9.1 第一阶段：基础架构（核心同步功能）✅ 已完成
- [x] 实现LocalFileSyncService基础服务
- [x] 实现FolderManager文件夹管理（基于v3.1架构）
- [x] 建立文件命名规范（通用资料/物料资料分类）
- [x] 实现基础的同步机制（上传/删除/供应商同步）
- [x] 实现事务性和一致性保证机制

**✅ 第一阶段完成总结**：
- ✅ LocalFileSyncService核心服务已实现并通过测试
- ✅ v3.1架构的文件夹管理已实现（通用资料/物料资料两层结构）
- ✅ 文件命名规范已建立并应用
- ✅ 上传/删除/供应商同步功能已实现
- ✅ 72个供应商文件夹结构已成功创建
- ✅ 前端同步供应商按钮已修复并正常工作
