...
**问题背景**：
- 适配器清理时发现全局实例名称不一致
- 使用 `window.uiUtils` 调用，但UI工具层创建的是 `window.supplierUIUtils`
- 导致所有提示功能失效

**架构决策分析**：
1. **选项1：window.supplierUIUtils** ✅
   - 命名空间明确，一看就知道属于供应商模块
   - 避免未来与其他模块的uiUtils冲突
   - 模块化清晰，可扩展性好
   - 调试友好，问题定位快速

2. **选项2：window.uiUtils** ❌
   - 调用简洁但容易冲突
   - 模块边界模糊
   - 全局污染风险
   - 维护难度高

**最终选择**：采用 `window.supplierUIUtils`

**实施措施**：
- ✅ 更新45个调用点：`window.uiUtils.xxx()` → `window.supplierUIUtils.xxx()`
- ✅ 移除ui-utils.js中的别名创建
- ✅ 确保无残留调用

#### **架构优化效果**
- ✅ **命名空间清晰**：所有UI调用明确属于供应商模块
- ✅ **避免冲突风险**：为未来其他模块预留空间（iqcUIUtils、commonUIUtils等）
- ✅ **调试友好**：错误定位更加精确
- ✅ **可维护性提升**：模块边界清晰
